...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (status != ERROR_SUCCESS) {
		log_dbg("[-] Can't create service key");
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		...
		log_dbg("[-] Can't create 'ImagePath' registry value");
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		log_dbg("[-] Can't create 'Type' registry value");
		return false;
	}
	...
	if (!NT_SUCCESS(Status)) {
		log_dbg("Fatal error: failed to acquire SE_LOAD_DRIVER_PRIVILEGE. Make sure you are running as administrator.");
		return false;
	}
	...
	log_dbg("[+] NtLoadDriver Status 0x%X", Status);

	if (Status == 0xC0000603) { //STATUS_IMAGE_CERT_REVOKED
		log_dbg("[-] Your vulnerable driver list is enabled and have blocked the driver loading, you must disable vulnerable driver list to use kdmapper with intel driver");
		log_dbg("[-] Registry path to disable vulnerable driver list: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\CI\\Config");
		log_dbg("[-] Set 'VulnerableDriverBlocklistEnable' as dword to 0");
	}
	else if (Status == 0xC0000022 || Status == 0xC000009A) { //STATUS_ACCESS_DENIED and STATUS_INSUFFICIENT_RESOURCES
		log_dbg("[-] Access Denied or Insufficient Resources (0x", Status, "), Probably some anticheat or antivirus running blocking the load of vulnerable driver");
	}
...
bool service::StopAndRemove(const std::wstring& driver_name) {
	...
	log_dbg("[+] NtUnloadDriver Status 0x%X", st);
	if (st != 0x0) {
		log_dbg("[-] Driver Unload Failed!!");
