...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			log_dbg("[-] Failed to load ntdll.dll" ); //never should happens
			return false;
		}
		...
		if (!NtAddAtom)
		{
			log_dbg("[-] Failed to get export ntdll.NtAddAtom" );
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		if (!kernel_NtAddAtom) {
			log_dbg("[-] Failed to get export ntoskrnl.NtAddAtom" );
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			log_dbg("[-] FAILED!: The code was already hooked!! another instance of kdmapper running?!" );
			return false;
		}
